...
I implemented the UI for a desktop version of the classic board game Battleship, where the user plays against a computer player. This assignment was for my User Interfaces course and focused on graphics, animations, hit-testing, and transformations.
...
1. **Setup**: Players place their ships on their board.
2. **Game loop**: During their turn, the player picks a square on their opponent's board to fire at. If a ship is in that square, they can fire again, otherwise their turn ends. A ship sinks when all the squares it's in has been fired at.
3. **Ending**: A player wins once they sink all of their opponent's ships.

## Features
### Setup
The UI consists of two 10 x 10 grids, called boards. The left board represents the user's (hereafter referred to as "the player") board. The right board represents the player's view of the opponent's board.

The player's ships are initially located between the boards, represented by rectangles.

The player uses the left mouse button to pick up and put down a ship. Once picked up, the player can move it around and use the right mouse button to toggle between orientations. When a ship is placed on the board, it will snap to the closest position. If the location isn't valid (e.g. there's a ship in the way or it's not on the board), then the ship will return to its initial position.
